...
def create_fhs_env_drv_flake(program: str) -> str:
    path = Path(program).expanduser()
    libs = find_libs(path)
...
def main(args=sys.argv[1:]):
    ...
    if parsed_args.destination:
        ...
        destination = get_cache_path(parsed_args.program) / "fhs-env" / filename
    ...
    if not destination.exists():
        ...
        if parsed_args.flake:
            fhs_shell = create_fhs_env_drv_flake(parsed_args.program)
        else:
